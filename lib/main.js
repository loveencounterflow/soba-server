...
(function() {
  var SOBA, TEMPLATES, TRM, alert, app, badge, debug, help, info, new_app, new_router, new_sio_server, njs_http, njs_path, router, rpr, sb, urge, warn, whisper;
  ...
  this.emit_news = function(me, topic, data) {
    return me['%sio-server'].emit('news', topic, data);
  };
  ...
  this._get_monitor = (function(_this) {
    return function() {
  ...
  this.serve = function(me) {
    return me['%http-server'] = me['%http-server'].listen(me['port'], function() {
      var family, host, port, _ref;
      help("server process running on Node v" + process.versions['node']);
      _ref = me['%http-server'].address(), host = _ref.address, family = _ref.family, port = _ref.port;
